...
const Button = ({ text, icon, isPrimary, type, location, handleEvent }) => {
  ...
  const buttonLocation = location ? `button__icon--${location}` : "button--default";

  const buttonClass = icon ? `button__icon ${buttonLocation}` : `button ${isPrimaryClass} ${buttonLocation}`;
  return (

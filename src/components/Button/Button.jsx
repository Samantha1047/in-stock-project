...
const Button = ({ text, icon, isPrimary, type, location }) => {
  ...
  const isPrimaryText = isPrimary ? "button--primary-text" : "button--secondary-text";

  const buttonLocation = location === "form" ? "button--form" : "button--default";

  //Might need to be adjusted if we include edit button
  return (
    <button type={type} className={`button ${isPrimaryClass} ${buttonLocation}`}>
